...
- [x] Verify: `pnpm exec vitest run` → 278 passed (was 277; +1 new contract test). `pnpm exec svelte-check` → 0 errors, 0 warnings. `pnpm exec vite build` → succeeds (confirms gated exclude doesn't break prod). `pyve test` → **411 passed** after bootstrapping the testenv editable install (`pyve testenv run pip install -e .`) — the testenv had been reinitialized recently and the project package + runtime deps (click, pydantic, pyyaml) needed to be re-installed per [docs/project-guide/go.md](../project-guide/go.md) § "Editable install and testenv dependency management." No Python-side regression from the fix.
