...
- [x] `docs/specs/nbfoundry/consumer-dependency-spec.md`: replaced the Option-B compile contract with **Option C**. Rewrote the "Design Decision" section (Option C is v1; Option B marked SUPERSEDED; Option A future/PyTorch-blocked) and **BR-1** — `compile_exercise(yaml_path, base_dir) -> {title, description, hints, environment, notebook_source}`, dropped `sections`/`expected_outputs`/`submission`/`instructions`, added the explicit *codegen MUST NOT import torch/modelfoundry; torch is learner-runtime only* constraint. **Contract-hygiene pass** (post-decision): `mode` is **not** in nbfoundry's dict — it is the author's `ExerciseBlock` field; spelled out `notebook_source` requirements; marked BR-4/BR-5 **removed** and RR-* as LF-frontend-only (not nbfoundry's contract); demoted internal story IDs to a "Contract history" pointer (A/B/C rationale kept).
...
- [x] `src/learningfoundry/resolver.py` (`ready` exercise): added `ExerciseArtifact` + `_DEFAULT_MARIMO_PORT=2718` + `ResolvedCurriculum.exercises`, threaded an `exercises` aggregator through module/lesson/block. The `ready` branch (a) pops `notebook_source` out of the content (browser never needs the `.py`) into an `ExerciseArtifact(id, notebook_source, notebook_path="exercises/<id>/<id>.py", mode=block.mode, port)`; (b) injects `id`/`status`/`mode`/`port` onto the content for the banner; (c) **retired the K.e `assets` aggregation loop**. Stub exercises emit no artifact.
- [x] `src/learningfoundry/generator.py`: added `_write_exercises` — writes each `notebook_source` to `output_dir/exercises/<id>/<id>.py` (**outside** `static/`) and the `exercises-manifest.json` sidecar (`id → {notebook_path, mode, port}`) at the project root; strips `exercises` from curriculum.json (like `assets`).
- [x] **K.e asset-staging fate: RETIRED.** `static/exercises/<id>/` image staging is removed with the static renderer (no `assets` list in the Option-C dict; marimo renders its own outputs at run time). Removed `static/exercises` from `_PRESERVED_PATHS` and deleted the K.e asset-staging tests. Notebooks are **not** preserved across rebuilds — they're regenerated every build, and only the author runs `build` (the learner runs the cloned output, so marimo edits are never clobbered).
